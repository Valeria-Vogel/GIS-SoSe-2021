...
function anzeige2(_auswahl: Darstellung): void {
 let anzeige: HTMLDivElement = <HTMLDivElement>document.getElementById("auswahlBereich");
 if (document.querySelector("title").getAttribute("id") == "body") {
     for (let i: number = 0; i < _auswahl.body_b.length; i++) {
         let div: HTMLDivElement = bodyDiv2(_auswahl.body_b[i]);
         anzeige.appendChild(div);
     }
 }
 else if (document.querySelector("title").getAttribute("id") == "hair2") {
     for (let i: number = 0; i < _auswahl.hair2.length; i++) {
         let div: HTMLDivElement = bodyDiv(_auswahl.hair2[i]);
         anzeige.appendChild(div);
     }
 }
 else if (document.querySelector("title").getAttribute("id") == "outfit2") {
     for (let i: number = 0; i < _auswahl.outfit2.length; i++) {
         let div: HTMLDivElement = bodyDiv2(_auswahl.outfit2[i]);
         anzeige.appendChild(div);
     }
 }
}
